...
/// <include file='docs/SessionConfigurator.xml' path='docs/members[@name="SessionConfigurator"]/SessionConfigurator/*'/>
	...
	public static class SessionConfigurator {
		...
		private static extern System.IntPtr dlopen(string filename, int flags);

		static SessionConfigurator() {
			if (OS.GetName() == "X11") {
				try {
					string libPath = System.IO.Path.Combine(System.IO.Directory.GetCurrentDirectory(), "libonnxruntime.so");
					GD.Print("[GodotONNX] Forcing native DL open: " + libPath);
					dlopen(libPath, 2); // RTLD_NOW
				} catch (System.Exception e) {
					GD.PrintErr("[GodotONNX] dlopen patch failed: " + e.Message);
				}
			}
		}
...
static public void SystemCheck()  
	{
		...
		string computeName = ComputeAPIID >= 0 && ComputeAPIID < ComputeNames.Length ? ComputeNames[ComputeAPIID] : "CPU";
		GD.Print("OS: " + OSName, " | Compute API: " + computeName);
