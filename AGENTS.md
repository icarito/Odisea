...
- **`shader_compilation_mode.iOS=0` es un contrato**: async+ubershader (modo ≥1) en GLES3 falla silenciosamente en los GL móviles — en iOS esconde meshes sin loguear nada (Pilot_V2 y los `.glb` invisibles, log limpio). En Android `.Android=0` es precaución (re-evaluar con FBO corregido).
- **`framebuffer_allocation.Android=3` es un contrato**: forzar "3D" (2) en Adreno tilea el framebuffer completo (mosaico de franjas repetidas); `USAGE_3D_NO_EFFECTS` lo arregla al instante (verificado con A/B en Redmi Note 9 Pro: 39 fps, draw calls 196→87). iOS tolera `2`.
- Diagnóstico rápido en device: Opciones → Overlay de log (lee `user://logs/godot.log`). El boot siempre imprime `Async. shader compilation: ...` y `Shader cache: ...` — esa línea dice qué camino tomó el driver. iOS exporta siempre en release (comandos remotos ANNAV2 bloqueados): el diagnóstico es el overlay, no telemetría. Android es build debug: `adb reverse tcp:4999 tcp:4999` + relanzar da control total en vivo (`adb logcat` para el log completo del motor).
...
### 11.10 Renderer GLES3 en Mali handhelds via FRT (confirmado en device, 2026-09-14)

- **Síntoma**: mundo 3D blanco/cian lavado con el HUD dibujando encima, 79 objetos en frame según `INFO_OBJECTS_IN_FRAME`, log limpio. El menú (Canvas) y Dome_Prologue (env InteriorLab sin post-process) renderizan perfecto — solo fallan los levels cuyo `Environment` habilita **fog + glow + DOF + tonemap ACES** (`Environment_DomeIntro.tres`). El color plano de la pantalla era el `fog_color` del env.
- **Contrato**: en Mali GLES3 el camino de post-process del Environment no es confiable via FRT/EGL. `GLES3VendorGate.gd` (autoload, scope por vendor "Mali" via `get_video_adapter_name/vendor`) apaga `fog/glow/dof_blur/adjustment` y fuerza `tonemap_mode=LINEAR` en cualquier `WorldEnvironment` que entre al árbol. Desktop e iOS no matchean el gate.
- **`Async. shader compilation` ya se auto-apaga en Mali** ("enabled for project, but not supported") — sync compile es lo esperado; el shader cache persiste en `conf/godot` del port.
- **Protocolo de A/B en device**: un toggle live de Environment NO resetea el estado GL — cada hipótesis se prueba con **reboot limpio del juego**, nunca toggleando en vivo. Diagnóstico vía binario FRT **debug** (`godot.box3d.frt.arm64.debug` del release del fork, mismo commit que el .release) + peer local con `ANNA_V2_BRIDGE=<ip_desktop>:4999` en `dev.sh` (el discovery UDP window de 1s vs broadcast de 5s es una carrera; el env es determinista).
