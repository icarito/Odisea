...
func _print_usage():
	print("Usage: godot3-bin -s tools/export_scene_geometry.gd --scene <path> [options]")
	print("Options:")
...
func _run_export():
	...
	# Procedural scaffold: find ScaffoldStreamController, set seed, force bake of chunk (0,0)
	if scene_name == "ScaffoldOrbit":
		print("Detected ScaffoldOrbit — force-baking base chunk with seed ", scene_seed)
		var ssc = _find_node_of_class(root, "ScaffoldStreamController")
		if ssc:
			ssc.global_seed = scene_seed
			ssc.streaming_enabled = true
			# Force the origin chunk to be processed
			# Give it a few frames to generate WFC/MST chunk (0,0)
			var start_msec = OS.get_ticks_msec()
			while OS.get_ticks_msec() - start_msec < 3000:
				# Pump idle/process to let the streamer generate
				if ssc.has_method("_process"):
					ssc._process(0.016)
				if _ssc_has_chunk_zero(ssc):
					break
				OS.delay_msec(50)
			if _ssc_has_chunk_zero(ssc):
				print("Chunk (0,0) generated, sampling active chunks")
			else:
				printerr("Warning: Could not force-bake chunk (0,0) for ScaffoldOrbit; sampling whatever is present")
...
func _sample_csg_shape(csg: CSGShape, points: Array, min_b: Vector3, max_b: Vector3):
	...
	# For CSGBox, CSGCylinder, CSGSphere: sample corners/boundary
	if csg is CSGBox:
	...
	elif csg is CSGCylinder:
		var r = max(csg.radius, 0.1)
		var h = max(csg.height, 0.1) * 0.5
		var radial_steps = max(int(r / resolution), 4)
		for i in range(radial_steps):
			var angle = i * TAU / radial_steps
			for y_sign in [-1, 1]:
				var p = gt.xform(Vector3(cos(angle) * r, y_sign * h, sin(angle) * r))
				points.append([stepify(p.x, 0.1), stepify(p.y, 0.1), stepify(p.z, 0.1)])
				_update_bounds(p, min_b, max_b)
	elif csg is CSGSphere:
		var r = max(csg.radius, 0.1)
		var steps = max(int(r / resolution), 4)
		for i in range(steps + 1):
			var phi = i * PI / steps
			for j in range(steps * 2):
				var theta = j * TAU / (steps * 2)
				var p = gt.xform(Vector3(sin(phi) * cos(theta) * r, cos(phi) * r, sin(phi) * sin(theta) * r))
				points.append([stepify(p.x, 0.1), stepify(p.y, 0.1), stepify(p.z, 0.1)])
				_update_bounds(p, min_b, max_b)
	else:
		# Generic CSG: sample AABB
		...
		if aabb:
			...
			for x in [-1, 1]:
				for y in [-1, 1]:
					for z in [-1, 1]:
						var p = gt.xform(aabb.position + Vector3(x * extents.x, y * extents.y, z * extents.z))
						points.append([stepify(p.x, 0.1), stepify(p.y, 0.1), stepify(p.z, 0.1)])
						_update_bounds(p, min_b, max_b)
...
func _ssc_has_chunk_zero(ssc: Node) -> bool:
	if ssc.has_method("_active_chunks"):
		return ssc.get("_active_chunks").has(Vector2(0, 0))
	return false
