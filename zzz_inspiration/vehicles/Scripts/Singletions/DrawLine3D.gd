...
class Line:
	...
	func _init(Start, End, LineColor, MTime):
		self.Start = Start
		self.End = End
...
func _draw():
	...
	#Remove lines that have timewd out
	var i = Lines.size() - 1
	while (i >= 0):
		if(Lines[i].Time < 0.0):
			Lines.remove(i)
...
func DrawLine(Start, End, LineColor, MTime = 0.0):
	Lines.append(Line.new(Start, End, LineColor, MTime))

func DrawRay(Start, Ray, LineColor, MTime = 0.0):
	Lines.append(Line.new(Start, Start + Ray, LineColor, MTime))

func DrawCube(Center, HalfExtents, LineColor, MTime = 0.0):
	...
	DrawLine(LinePointStart, LinePointEnd, LineColor, MTime);
	...
	#Draw vertical lines
	LinePointStart = LinePointEnd
	DrawRay(LinePointStart, Vector3(0, HalfExtents * 2.0, 0), LineColor, MTime)
