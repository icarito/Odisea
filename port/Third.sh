...
GODOT_OPTS="--remote-debug 192.168.18.6:6007"
...
if [[ "$CFW_NAME" = "ROCKNIX" ]]; then
    ...
    if ! glxinfo | grep -q "OpenGL version string"; then
        ...
        "$runtime" $GODOT_OPTS --video-driver GLES2 --main-pack "third.pck"
    else
        # Using Panfrost or Freedreno driver
        "$runtime" $GODOT_OPTS --main-pack "third.pck"
    fi
else
