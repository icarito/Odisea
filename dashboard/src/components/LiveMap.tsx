  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (canvas && ctx) {
        ...
        // --- Scene Geometry (Birdseye) ---
        if (sceneRef.current !== '' && geometry) {
          ctx.save();
          // Draw point cloud (contorno)
          ctx.fillStyle = 'rgba(127, 209, 255, 0.15)';
          ...
          geometry.zones.forEach(z => {
            const x1 = toX(z.bounds.min[0]);
            const z1 = toZ(z.bounds.min[2]);
            const x2 = toX(z.bounds.max[0]);
            const z2 = toZ(z.bounds.max[2]);
          ...
          geometry.props.forEach(p => {
            const sx = toX(p.position[0]);
            const sz = toZ(p.position[2]);
            const isInteractable = p.tags.includes('interactable');
