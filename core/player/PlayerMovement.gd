...
# PlayerMovement.gd - Componente para movimiento horizontal suavizado
...
func set_floor_normal(normal: Vector3) -> void:
	"""Called by PlayerController after move_and_slide to update floor info."""
...
func process_movement(dt: float, move_vec: Vector2, basis: Basis, sprint: bool, is_on_floor: bool, crouch: bool = false) -> void:
	...
	# print("[PlayerMovement] move_vec=%s forward=%s wish_dir=%s" % [move_vec, forward, wish_dir])
	...
	# print("[PlayerMovement] h_vel=%s" % horizontal_velocity)
