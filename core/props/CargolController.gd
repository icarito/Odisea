...
# CargolController.gd
# Links an InteractableBase (e.g. Lever) to a CargolDrone.
# Activate → drone follows the player.
# Deactivate → drone returns to home position.
# Attach as a child of the Lever/InteractableBase node.
...
func _ready():
	...
	# The parent should be an InteractableBase (or InteractableBridge with sources)
