...
# CargolDroneProp.gd
# PropZoo-compatible wrapper for CargolDrone.
...
func _ready():
	...
	if _drone:
		...
		printerr("[CargolDroneProp] No CargolDrone child found!")
...
func _find_drone(node: Node):
	"""Recursively find CargolDrone among children."""
	for child in node.get_children():
		if child is CargolDrone:
			return child
