...
# Debe ser conectado al PlayerController y escuchar input.
...
func _respawn_at_spawn_or_zero():
	...
	if player_controller and player_controller.is_inside_tree():
		...
		var pilot_scene = preload("res://core/actors/Pilot.tscn")
...
func _on_player_killed():
	...
	# 3. Si hay SpawnPoint en la escena, usarlo
	if not target_transform:
		var spawn = get_tree().current_scene.find_node("SpawnPoint", true, false) if get_tree().current_scene else null
		print("[TeleportSystem] Buscando SpawnPoint:", spawn)
		if spawn:
			target_transform = spawn.global_transform
			print("[TeleportSystem] Respawn usando SpawnPoint.")
		...
		if sm:
					...
					if sm.has_method("get") and sm.get("_oys_input_override") and not sm.get("_oys_input_override").empty():
						...
						if od.has("interact") and od.get("interact"):
							has_active = true
						if has_active:
							var InputData = preload("res://core/input/InputData.gd")
							var idata = InputData.new()
							idata.from_dict(od)
