...
func test_props_load_and_instantiate():
	var runner = scene_runner("res://core/tests/TestScene_SciFiLights.tscn")
	yield(runner.simulate_frames(10), "completed")

	var floating_light = runner.scene().get_node("SciFiFloatingLight")
	var static_light = runner.scene().get_node("SciFiStaticLight")
	var floor_panel = runner.scene().get_node("SciFiFloorPanel")
