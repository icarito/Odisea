...
const AudioManagerScript = preload("res://core/autoloads/AudioManager.gd")
const BGMZoneScript = preload("res://core/components/BGMZone.gd")

func test_bgm_zone_inheritance():
	var zone = BGMZoneScript.new()
	assert_object(zone).is_instanceof(BaseZone)
...
func test_audio_manager_sorting():
	...
	# BaseZone uses zone_extents for get_volume()
