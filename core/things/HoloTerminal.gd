...
class_name HoloTerminal

# HoloTerminal.gd - Interactive Holographic Screen
# Inherits InteractableBase for replay determinism.
...
var _terminal_ui = null # TerminalUI reference
...
func _setup_cinematic_camera() -> void:
	...
	if not _camera_zone:
		if use_cinematic_zone:
			print("[HoloTerminal] Warning: CameraZone not found in CinematicSetup")
		return
	...
	if not _cinematic_rig:
		if use_cinematic_zone:
			print("[HoloTerminal] Warning: CinematicPathRig not found in CinematicSetup")
...
func _input(event):
	...
	if is_ui_interactive() and allow_focus_mode:
		if event.is_action_pressed("focus") and not _is_focused:
			print("[HoloTerminal] Focus key pressed, entering focus mode")
			_enter_focus_mode()
			get_tree().set_input_as_handled()
			return
		elif event.is_action_pressed("ui_cancel") and _is_focused:
			print("[HoloTerminal] Cancel key pressed, exiting focus mode")
			_exit_focus_mode()
			get_tree().set_input_as_handled()
			return
...
func _enter_focus_mode():
	...
	if not _focused_rig:
		print("[HoloTerminal] Cannot enter focus mode: FocusedRig not found")
		return
	
	_is_focused = true
	print("[HoloTerminal] Entering focus mode, activating FocusedRig")
...
func _exit_focus_mode():
	...
	if not use_cinematic_zone:
		print("[HoloTerminal] Exiting focus mode, returning to regular camera")
		if _camera_zone and "is_zone_active" in _camera_zone:
			_camera_zone.is_zone_active = false
	...
	if not _cinematic_rig:
		print("[HoloTerminal] Cannot exit focus mode: CinematicPathRig not found")
		return
	
	print("[HoloTerminal] Exiting focus mode, activating CinematicPathRig")
