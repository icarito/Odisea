...
# PressurePlate.gd - Deterministic Weight-based Switch
...
func interact() -> void:
	if debug or Engine.editor_hint:
		...
		print("[PressurePlate] Debug override toggled: ", _debug_weight_override)
		return
	.interact()
...
func _ready():
	...
	if has_node("Detector"):
		...
		printerr("[PressurePlate] Error: 'Detector' Area node not found! Collision logic will fail.")
...
func step(dt: float) -> void:
	...
	if _detection_area:
		...
		# if bodies.size() > 0: print("[PressurePlate] Detect: ", bodies)
