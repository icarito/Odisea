...
# PushableBox.gd - Hybrid Deterministic Object
...
func _settle():
	if debug:
		print("[PushableBox] Settling at ", global_transform.origin)
...
func set_external_velocity(vel):
	...
	if mode == RigidBody.MODE_RIGID:
		...
		if debug and force.length() > 0.1:
			print("[PushableBox] set_ext_vel: force=%s" % force)
