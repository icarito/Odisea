...
func _on_zone_entered(body: Node):
	...
	print("[OcclusionZone] Body entered: ", body.name, " Active count: ", WallOcclusionManager.registered_materials.size())
...
func _apply_enforcement():
	...
	if not root:
		print("[OcclusionZone] Target path invalid: ", occlusion_target_path)
		return
		
	var shader_to_use = custom_occlusion_shader
	if not shader_to_use:
		shader_to_use = load("res://shaders/dither_hiding.shader")
	print("[OcclusionZone] Applying enforcement starting from target: ", root.name)
