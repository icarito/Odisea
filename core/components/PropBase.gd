...
func _ready():
	# Note: InteractableBase._ready() is called automatically in Godot 3
	_auto_wire_switches()
	...
	for path in visual_parts_paths:
		...
		if node:
			print("[PropBase] Caching visual part: ", node.name, " (", node.get_path(), ")")
			_visual_parts.append(node)
		else:
			print("[PropBase] WARNING: Visual part not found at path: ", path)
...
func _update_visuals() -> void:
	if debug:
		print("[PropBase] _update_visuals: anim_progress=", anim_progress, " is_active=", is_active)
