...
# WindTunnel.gd - Deterministic Wind Force
# Applies external velocity to supported bodies (like PlayerController).
	...
	for body in bodies:
		if body.has_method("set_external_velocity"):
			...
			if body.has_method("set_external_source_is_static"):
				...
				# This ensures PlayerController applies it even when on the floor (overcoming friction)
