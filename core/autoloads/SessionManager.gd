...
func _ready():
	...
	if not has_node("TeleportSystem"):
		var ts_res = load("res://core/systems/TeleportSystem.gd")
		if ts_res == null:
			printerr("SessionManager: Could not load TeleportSystem resource: res://core/systems/TeleportSystem.gd")
		else:
...
func _connect_teleport_system():
	...
	# Validar y reconectar InputProvider
	if is_instance_valid(player_node):
		var _unused_provider = player_node.get_node_or_null("InputProvider")
...
func _on_tree_changed_for_script(script_path: String):
	...
	var OYS_Interpreter = load("res://core/systems/OYS_Interpreter.gd")
...
func load_and_play(path: String):
	...
	if ext == "oys":
		...
		# Find/Load Level
		var scene_path = "res://core/levels/TestScene.tscn"
...
func run_simulation_from_buffer(buffer_data: Array, world_start_state: Dictionary, player_controller: Node, yield_frames: int = 3) -> Dictionary:
	...
	# 3. Crear InputProvider en modo REPLAY
	var InputProvider = preload("res://core/input/InputProvider.gd")
	var input_provider = InputProvider.new()
	var input_buffer = []
