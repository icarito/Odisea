...
animate = true; // true para usar $t (Animate), false para usar anim_value
anim_value = 0; // rango 0..1 cuando animate == false (0:Axial, 1:Centrífugo)
animation_easing = "inout"; // "linear" | "smooth" | "inout"
...
function ease(x) =
  animation_easing == "linear" ? ease_linear(x)
...
interlock_angle_deg = 60; // ajustable
