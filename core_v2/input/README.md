...
The `PlayerControllerV2` owns an `InputProviderV2`.
In `_physics_process`:
1. Player calls `provider.get_input()`.
2. Provider checks strictly current hardware state (or replay buffer).
3. Player passes `InputDataV2` to its `step(dt, input)` method.
...
- **Camera (Right Stick)**: Computed manually in `InputProviderV2` by reading `JOY_AXIS_2` (X) and `JOY_AXIS_3` (Y).
- **Deadzone**: A hardcoded constant `JOY_DEADZONE` (0.2) is applied.

## Future: Local Multiplayer

To support multiple local players:
1. Instantiate multiple `InputProviderV2` instances.
2. Add a `device_id` property to `InputProviderV2`.
3. Update `Input.get_joy_axis(device_id, ...)` calls to use this dynamic ID.
4. Assign each provider to a different `PlayerControllerV2`.
