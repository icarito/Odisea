...
func test_baseterrace_work_light_matches_trenchbroom_overrides():
	...
	var tripod: Node = scene.get_node_or_null("WorldRotator/Terrace/Building/QodotMap/entity_4_light_work_tripod")
	assert_object(tripod).is_not_null()
...
func test_baseterrace_holo_terminal_keeps_single_viewport_input_tree():
	...
	var terminal: Node = scene.get_node_or_null("WorldRotator/Terrace/Building/TableTerminal")
...
func test_baseterrace_vcamera_system_keeps_single_camera_tree():
	...
	var scene: Node = packed.instance()
	var vcam_system: Node = scene.get_node_or_null("WorldRotator/Terrace/Building/VCameraSystem")
...
func test_baseterrace_start_criopod_keeps_single_player_audio_listener():
	...
	var scene: Node = packed.instance()
	var criopod: Node = scene.get_node_or_null("WorldRotator/Terrace/Building/CrioPod START")
	assert_object(criopod).is_not_null()
	assert_int(_count_direct_children_named(criopod, "Pilot")).is_equal(0)
	...
	var pilot: Node = scene.get_node_or_null("Pilot")
	assert_object(pilot).is_not_null()
	assert_int(_count_direct_children_named(scene, "Pilot")).is_equal(1)
	assert_int(_count_descendants_named(criopod, "Pilot")).is_equal(0)
	assert_int(_count_direct_children_named(pilot, "AudioListener")).is_equal(1)
	assert_int(_count_descendants_of_class(scene, "Listener")).is_equal(1)
