...
const PIECES = [
	{"name": "E EndCap CSG", "path": "res://core_v2/props/duct/DuctEndCap.tscn", "connections": [true, false, false, false], "rotation": 0},
	{"name": "W Radial CSG", "path": "res://core_v2/props/duct/DuctRadial.tscn", "connections": [true, false, true, false], "rotation": 0},
	{"name": "W Circum Mesh", "path": "", "connections": [false, true, false, true], "rotation": 90},
	{"name": "S Incline CSG", "path": "res://core_v2/props/duct/DuctIncline.tscn", "connections": [true, false, true, false], "rotation": 0},
	{"name": "C Elbow CSG", "path": "res://core_v2/props/duct/DuctElbow.tscn", "connections": [true, true, false, false], "rotation": 0},
	{"name": "T Tee CSG", "path": "res://core_v2/props/duct/DuctTee.tscn", "connections": [true, true, true, false], "rotation": 90},
	{"name": "X Cross CSG", "path": "res://core_v2/props/duct/DuctCross.tscn", "connections": [true, true, true, true], "rotation": 0},
	{"name": "Room Capsule CSG", "path": "res://core_v2/props/duct/CapsuleRoom.tscn", "connections": [true, true, false, true], "rotation": 0}
]
...
func _make_piece(spec: Dictionary) -> Spatial:
	if String(spec.get("path", "")) == "":
		return _make_arc_piece()
	var scene = load(String(spec.get("path", "")))
	if not scene:
		return null
...
func _build_piece_gallery() -> void:
	for i in range(PIECES.size()):
		...
		var piece_name: String = String(spec.get("name", "Unknown"))
		var root: Spatial = Spatial.new()
		root.name = "Piece_%02d_%s" % [i, piece_name]
		var column: int = i % 4
		var row: int = i / 4
		root.translation = Vector3(float(column) * 22.0 - 33.0, 0.0, float(row) * 24.0)
		add_child(root)
		...
		if instance:
			...
			_force_dual_sided(instance)
		_add_label(root, piece_name)
		_draw_local_axes(root)
		_draw_ports(root, spec.get("connections", [false, false, false, false]), _ring_radius(0))

func _add_label(parent: Spatial, text: String) -> void:
	...
	label.set("outline_size", 18)
	label.set("outline_modulate", Color(0.0, 0.0, 0.0, 1.0))
	label.translation = Vector3(0, 12.0, 0)
	parent.add_child(label)

func _build_polar_preview() -> void:
	var spawner = DuctMazeStreamerScript.new()
	spawner.name = "PolarMazePreview"
	spawner.translation = Vector3(0, 0, -112)
...
func _force_dual_sided(node: Node) -> void:
	if node is MeshInstance:
		var mesh_instance: MeshInstance = node
		if mesh_instance.material_override:
			mesh_instance.material_override = _dual_sided_material(mesh_instance.material_override)
		if mesh_instance.mesh:
			for i in range(mesh_instance.mesh.get_surface_count()):
				var mat: Material = mesh_instance.get_surface_material(i)
				if not mat:
					mat = mesh_instance.mesh.surface_get_material(i)
				if mat:
					mesh_instance.set_surface_material(i, _dual_sided_material(mat))
	var csg_material = node.get("material")
	if csg_material is Material:
		node.set("material", _dual_sided_material(csg_material))
	for child in node.get_children():
		_force_dual_sided(child)

func _dual_sided_material(material: Material) -> Material:
	if material is SpatialMaterial:
		var mat: SpatialMaterial = material.duplicate()
		mat.params_cull_mode = SpatialMaterial.CULL_DISABLED
		return mat
	return material
