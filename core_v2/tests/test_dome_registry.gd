...
func test_get_dome_id_for_plate_uses_explicit_then_synthetic_ids() -> void:
	assert_str(DomeRegistry.get_dome_id_for_plate(0, 1)).is_equal("dome_01")
	assert_str(DomeRegistry.get_dome_id_for_plate(0, 0)).is_equal("dome_intro")
...
func test_unassigned_domes_use_the_optimized_default_interior() -> void:
	# Dome_Default es la copia optimizada (rampas quitadas, piso como box, aspas
	# horneadas). Los domos sin interior propio deben apuntar ahí, no a Dome_01.
	assert_str(DomeRegistry.DEFAULT_INTERIOR_SCENE).is_equal("res://core_v2/levels/interiors/Dome_Default.tscn")
	for dome_id in ["dome_02", "dome_03", "dome_s03_p017"]:
		assert_str(String(DomeRegistry.get_dome(dome_id).get("interior_scene", ""))) \
			.is_equal("res://core_v2/levels/interiors/Dome_Default.tscn")

func test_intro_dome_keeps_its_own_interior() -> void:
	assert_str(String(DomeRegistry.get_dome("dome_intro").get("interior_scene", ""))) \
		.is_equal("res://core_v2/levels/interiors/Dome_Intro.tscn")
