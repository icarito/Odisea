shader_type spatial;
// Variante para plasma: la textura del atlas aporta FORMA, no color. El flipbook de gas
// es anaranjado, y multiplicar su RGB volvia amarillo cualquier tinte. Acá el color sale
// entero del material, que es lo que deja leer el chorro como plasma y no como fuego.
...
void fragment() {
	...
	float alpha = tex.a * instance_color.a * alpha_multiplier * age_fade;

	// La forma sale de la luminancia del atlas; el color, del color de la partícula.
	float shape = dot(tex.rgb, vec3(0.299, 0.587, 0.114));
	ALBEDO = instance_color.rgb * shape;
	EMISSION = instance_color.rgb * shape * mix(emission_strength, fire_emission_strength, particle_fire_mix);
