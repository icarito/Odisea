...
func _apply_boundary_constraints(domains: Dictionary, candidates: Array):
	for pos in candidates:
		# Anchors: sL at the start/end of the volume (X axis)
		if pos.x < 30.0 or pos.x > VOLUME_X - 30.0:
			# Force sL in boundaries if possible
			if "sL" in domains[pos]:
				domains[pos] = ["sL"]
...
func _propagate_room_constraints(domains: Dictionary, placed: Dictionary, collapsed_pos: Vector3):
	...
	for other_pos in domains:
		...
		# Increased propagation range to ensure constraints reach neighbors
		if dist > 60.0:
			continue
...
func _pick_connector(room_a: String, room_b: String, height_diff: float) -> String:
	# sJ and sD are usually at walkway level, but we follow height rules strictly
	if height_diff > 2.0:
		return "cR"
	return "cW"
