extends Spatial
class_name FireVisualBand

# FD-051: capa VISUAL del fuego. Cabalga sobre `fire_height` y nunca lo alimenta de vuelta.
#
# Reutiliza GasParticleManager (FD-045) configurado como banda de fuego: emite partículas en
# un arco anular a la altura de la línea de fuego, solo en el sector cercano a la cámara.
# El resto del anillo no gasta partículas. `collide_with_world` va en false (sin raycast por
# partícula) — ese es el techo de coste.
#
# Si por LOD no hay partículas en un sector, el fire_height lógico sigue matando ahí igual.
# Nada de esto entra al snapshot: se reconstruye desde el estado lógico.

# Radio exterior del fuego (pared del domo). El fuego cubre todo el disco interior hasta
# acá, no solo un anillo delgado: la superficie completa del piso bajo fire_height arde.
...
# como una superficie de lava completa.
...
# Ruido vertical de la línea de fuego (para que no sea plana).
export(float) var line_noise := 0.6
# Suavizado del seguimiento a fire_height.
export(float) var follow_lerp := 12.0
# Velocidad vertical inicial de las lenguas de fuego. Baja a propósito: cada llama debe
# nacer y apagarse cerca de fire_height, no viajar varios metros (eso lee como columnas
# verticales en vez de una banda horizontal).
export(float) var flame_rise_speed := 0.35
# Cuánto vive cada lengua de fuego individual (más corto = banda más plana y quieta).
export(float) var flame_lifetime := 0.55
# Escala visual de cada lengua de fuego (base; se varía por burbuja, ver flame_scale_variance).
export(float) var flame_scale := 5.5
...
# (lava) en vez de lenguas individuales separadas.
export(float) var flame_scale_variance := 0.5
# Amplitud del "oleaje" de la superficie: ondas continuas (seno) en vez de ruido puro por
# partícula, para que la línea de fuego suba y baje como una superficie de lava viva.
...
# Solo fuego: color de la lava más caliente (núcleo de una burbuja/lengua), casi blanco.
export(Color) var hot_color := Color(1.0, 0.85, 0.35, 0.95)
# Solo fuego: color de la lava más fría (borde/costra), rojo oscuro casi apagado.
export(Color) var cool_color := Color(0.55, 0.08, 0.02, 0.85)
...
# Modo corona de humo: emite por encima de fire_height (offset vertical), sin marcar
# combustión (usa el color normal del manager, no la vía de fuego del shader). Pensado
...
func _on_fire_height_changed(height: float) -> void:
...
func _emit_flame(center_angle: float, half_arc: float) -> void:
	_emit_counter += 1
	...
	if emit_center_column and (_emit_counter % 5) == 0:
		...
		# Disco completo, no solo el borde: toda la superficie del piso bajo fire_height
	...
	if is_smoke_crown:
		...
		# lava sube y baja en ondas continuas que se desplazan con el tiempo, más un
	...
	var velocity := Vector3(0.0, flame_rise_speed * (0.6 + r3 * 0.8), 0.0)
	var scale := flame_scale
	if is_smoke_crown:
		...
		# continua (lava) en vez de lenguas individuales separadas.
		scale = flame_scale * (1.0 - flame_scale_variance * 0.5 + r2 * flame_scale_variance)
	...
	if not is_smoke_crown:
		...
		# El shader usa la vía de combustión siempre: esto es fuego, no gas/humo.
