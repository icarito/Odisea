#!/usr/bin/env python3
"""
MAX SPEED TRAINING - Optimized for throughput
- Full curriculum: RL -> RL2 -> RL3_Door -> RL4  
- More training stats (value_loss, entropy, etc.)
- Larger batch/steps for efficiency
"""
import sys, os, time

LOG_FILE = '/tmp/train_max_speed.log'
        ...
        if self.n_calls % 200 == 0:
            ...
            if hasattr(self.model, 'logger') and self.model.logger:
                ...
                if 'train/policy_loss' in lv:
                    stats += f" | policy={lv['train/policy_loss']:.4f}"
                if 'train/value_loss' in lv:
                    stats += f" | value={lv['train/value_loss']:.4f}"
                if 'train/entropy_loss' in lv:
                    stats += f" | entropy={lv['train/entropy_loss']:.4f}"
                if 'train/approx_kl' in lv:
                    stats += f" | kl={lv['train/approx_kl']:.4f}"
                if 'train/explained_variance' in lv:
                    stats += f" | var={lv['train/explained_variance']:.2f}"
                if 'train/loss' in lv:
                    stats += f" | total_loss={lv['train/loss']:.4f}"
...
def main():
    ...
    model = PPO(
        'MlpPolicy', env,
        n_steps=4096,      # Larger for better throughput
    ...
    model.save(f"{MODELS_DIR}/max_speed_final.zip")
