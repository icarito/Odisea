...
**Fecha:** 2026-09-20
**Alcance:** `core_v2/ui/` (~10.400 líneas GDScript) + `core_v2/autoloads/SuitOS.gd`
**Base:** rama `feature/FD-309-interactable-hudable-placeholder`
**Autor:** Odiseo (consultor)
**Docs hermanos:** FD-310 (mapa de la GUI), FD-311 (faltantes por urgencia)
...
archivos concentran el 55% del código** (`HudModeOverlay` 910 + `SuitOSWidgetHost` 709 =
1.619 de 3.410 en `ui/hud/`) y **duplican entre sí la misma máquina de estados de
arrastre**. El refactor rentable es **extraer esa máquina duplicada y los hold/drag
...
- `# ponytail:` en `SuitOSWidgetHost.gd:663` reconoce que **el hold del host se mide con
...
| Migrar todo a `.tscn` de una | Hoy el wiring en `_enter_tree` funciona y es idempotente; migrar es churn sin beneficio inmediato. Documentar el patrón (FD-310 §5) y migrar pantalla por pantalla si molesta. |
| Unificar el HUD de OdiseaOS con la capa retro OYS | Son UIs distintas con propósitos distintos; unificarlas es decisión de producto (ver FD-311 D-1), no refactor. |
...
2. Resolver/registrar los drifts de FD-310 §7, al menos el #1 (modelo de slots), porque
   la Fase 2 toca la FSM que los implementa.
