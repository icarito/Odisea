...
* Spawn Logic: The emitter utilizes a PackedScene property to define the entity type (initially targeting PushableBoxV2.tscn).
...
* Target Filtering: The area must be configured with its collision_mask set to Layer 2 (designated for "things" in core_v2). It must further validate targets using class checks (e.g., is PushableBoxV2 or is InteractableBaseV2) to ensure internal player actors are not accidentally cleared.
...
Consistent with the "Generalizable Components" philosophy of core_v2/components, these areas are designed to be agnostic of the specific prop type, provided the prop inherits from InteractableBaseV2.

Initial Implementation: PushableBoxV2 The system will be validated in core_v2/levels/TestScene_PushableBox.tscn. By utilizing the base class for registration, the emitter can handle any future core_v2 prop without script modifications.

Current Static Implementation	Proposed Dynamic Implementation
Manual Placement: Every box requires manual placement in core_v2/levels/.	Automated Generation: Volumes handle density, reducing manual editor labor.
...
These components are essential to solidifying the core_v2 ecosystem, providing the infrastructure necessary for a robust, high-performance, and fully deterministic immersive simulation.
