...
G31 se satura por la carga de la escena por frame**. Panfrost no sirve como salida (se congela), y
tampoco decide si es un bug del blob. La escena simple
...
| Cambiar a Panfrost como salida | H6: Panfrost (Mesa), Sebastián | peor: el juego se congela. Falla distinto, así que **no** confirma ni descarta un bug del blob |
...
- **Panfrost no es salida:** con Panfrost el juego se congela, peor que con el blob. Como falla de
  otra forma, no sirve para decidir entre "bug de libmali" y "límite del G31". Un congelamiento
  también es compatible con saturación (Panfrost tiene su propio heap de tiler acotado), pero no se
  midió. En la práctica, el blob queda como el único driver y la salida es bajar la carga por frame.
...
3. ~~H6 Panfrost~~: probado por Sebastián, se congela (ver descartes). Se sigue con libmali.
