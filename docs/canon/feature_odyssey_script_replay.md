...
Guarda tu secuencia en `core/tests/mi_test.oys`:
...
```sh
godot3-bin --no-window --script ./core/utils/OYSRunner.gd ./core/tests/mi_test.oys
```
...
```
El sistema detecta y ejecuta todos los `.oys` y `.json` en `core/tests/` y subcarpetas.

- Si el archivo es `.oys`, se convierte en vivo y se usa la escena `TestScene.tscn` por defecto (o la que declares con `LEVEL ...`).
...
```sh
godot3-bin --main-pack project.pck --replay res://core/tests/mi_test.oys
```
O con un `.json`:
```sh
godot3-bin --main-pack project.pck --replay res://core/tests/mi_test.json
```

## 5. Buenas prácticas
- Usa comentarios y `SECTION` para claridad.
- Usa asserts para validar resultados esperados.
- Si tu OYS no declara `LEVEL`, se usará `TestScene.tscn`.
...
|-------------------------------|------------------------------------------------------------------------------|
| Escribir test OYS             | Edita `.oys` en `core/tests/`                                             |
| Generar JSON (opcional)       | `godot3-bin --no-window --script ./core/utils/OYSRunner.gd archivo.oys`   |
| Ejecutar todos los tests      | `./runtest.sh -a ./core/tests/test_determinism.gd`                     |
