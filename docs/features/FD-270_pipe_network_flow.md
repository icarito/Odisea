...
| LM5 | Instanciar `RandomLeakSeeder`, poblar `candidate_leak_paths` con las ~12 fugas de LM1/LM2 | LOCAL | `core_v2/levels/interiors/Dome_Intro.tscn` | 2-3 fugas arrancan activas, distintas entre seeds distintos | JM1, LM1, LM2 | **hecho** · commit `e4a46b26`. 24 candidatas (12 tramo recto + 12 anillo), seed=42, leak_count=2 |
| LM6 | Instanciar dials (JM2) + diagrama (JM3) en ambos `HoloTerminalV2` (`StatusTerminal`, `HangingDisplay`), cablear `room_path` al `Room3D` de LM4 | LOCAL | `core_v2/levels/interiors/Dome_Intro.tscn` | paneles visibles y legibles en captura de `run-odisea` | JM2, JM3, LM4 | **hecho** · commit `2ebbf440`. Solo `HangingDisplay` existe en Dome_Intro (no hay `StatusTerminal` separado); layout 3 columnas; `RoomDialsPanel.room_path` con NodePath absoluto por la frontera del Viewport |
