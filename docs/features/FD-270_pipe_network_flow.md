...
| J4 | **Fix H5**: en `CoolantLeak`, la transición a `DEPRESSURIZED` deja de depender solo de `valve_path` propio y pasa por el caudal del adapter (vía la señal que ya dispara J2, o `is_pressurized_at`) | JULES | `core_v2/systems/cryo/CoolantLeak.gd` | cerrar `ValveWest_2` (no la que apunta `valve_path`) también despresuriza `LeakWest` | J2 (despachada en paralelo contra el contrato de J2, no su resultado) | en curso · jules `17145893040621048022` |
