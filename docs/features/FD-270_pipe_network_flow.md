...
| J3 | **Autoridad del parche** (§4/T5/H6): `LeakPatchPoint.patch_with_gloo()` deja de leer `_manometer`, pregunta `_flow_adapter.is_pressurized_at(leak)`; unificar `firm_patch_pressure_threshold` a escala `0..1` | JULES | `core_v2/systems/cryo/LeakPatchPoint.gd` | `test_firm_patch_requires_zero_flow` pasa | J2 (despachada en paralelo contra el contrato de J2, no su resultado) | en curso · jules `17876440022092747161` |
| J4 | **Fix H5**: en `CoolantLeak`, la transición a `DEPRESSURIZED` deja de depender solo de `valve_path` propio y pasa por el caudal del adapter (vía la señal que ya dispara J2, o `is_pressurized_at`) | JULES | `core_v2/systems/cryo/CoolantLeak.gd` | cerrar `ValveWest_2` (no la que apunta `valve_path`) también despresuriza `LeakWest` | J2 (despachada en paralelo contra el contrato de J2, no su resultado) | PR publicado y verificado (3/3 tests nuevos, 5/5 existentes sin regresión) · jules `17145893040621048022`, **PR #294 pendiente de merge por Sebastián** (sin permisos de merge desde esta sesión) |
