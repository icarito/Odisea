# FD-312: Faltantes de la GUI de OdiseaOS — inventario priorizado
...
**Relacionadas:** `docs/engineering/gui-map-2026-09-20.md` (mapa de la GUI) · FD-304 · FD-305 · FD-306 · FD-307 · FD-308 · FD-309

## Propósito

Inventario **de una sola página** de todo lo que la GUI de OdiseaOS **no tiene todavía**,
clasificado por urgencia. Es el complemento del mapa de la GUI: el mapa dice "qué existe y cómo
funciona"; este doc dice "qué falta y cuándo importa".
...
| P0-2 | **Modo HUD local (F3 de FD-296): "en diseño, pendiente delegar".** El overlay existe y funciona, pero el estado F3 sigue marcado como no entregado. | F3 no cerrada | FD-296 §fases; el mapa §2.2 | Verificar con Sebastian si F3 se considera entregada por el merge de F1/F1.5 → cerrar la casilla o re-delegar |
| P0-3 | **Ninguna pantalla instancia su `HUDableComponent` desde `.tscn`.** Todo el wiring vive en código / `_enter_tree`. | 0 `.tscn` con `HUDableComponent` | recon: "No hay ningún `.tscn` que instancie `HUDableComponent`" | Es una decisión de patrón: dejar en código (documentar) o migrar a escenas. Ver el mapa §5 |
...
| P1-5 | **4 slots en el control remoto.** Hoy el teléfono usa A/B; §6/F4 y §"Fuera de alcance" de FD-296 se contradicen (drift #2 del mapa). | A/B en el teléfono | FD-296 §6/F4 vs §Out of scope | Decidir (ver Open Question, drift #2) |
...
| P2-4 | **Priorización/relevancia sin feedback al jugador:** `relevance()` ordena el radial, pero nada comunica *por qué* una pantalla subió. | No existe | el mapa §2.2 |
...
|---|---|---|
| D-1 | ~~Destino de la capa retro~~ — **RESUELTO 2026-09-20 (Sebastián): se promueve a diegética.** Pasa a ser la *superficie de a bordo* de OdiseaOS frente a la *superficie del traje*; cian = traje, verde/ámbar = nave. | Cerrado. El trabajo que genera (unificar marca, deshacer la colisión de nombre con OdysseyScript, pasar `RetroOS.tres` a tokens) entra en el plan de consolidación |
| D-2 | **Reconciliación de los 9 drifts entre FDs** (el mapa §7). En especial el #1 (modelo de slots A/B vs. 4 manuales) y el #2 (4 slots en el teléfono). | Los FDs son la spec; mientras se contradigan, cualquier implementación es "correcta" e "incorrecta" a la vez |
...
## Agregados el 2026-09-20 (verificados sobre `main` `be301b06`)

| # | Faltante | Estado hoy | Evidencia | Urgencia |
|---|---|---|---|---|
| P1-8 | **No hay lenguaje visual.** Ningún widget del HUD declara tema ni fuente; hay **47 colores distintos hardcodeados** en `core_v2/ui/hud/` (140 ocurrencias sumando `radial/` y `retro/`) y solo 11 constantes con nombre en 3 archivos. Conviven **tres paletas** sin fuente única: el cian del traje (sin `.tres`), `core_v2/ui/retro/RetroOS.tres` y el legacy `assets/themes/retro_scifi.tres`. | Sin tokens | `Color(0.5,0.5,0.5,0.8)` ("offline") copiado 11 veces; el mismo cian como `0.83` y `0.835`; `CargolWidget` con su propia escala de 7 colores que no coincide con el semáforo de `SystemStatusWidget` | **P1** |
| P1-9 | **Hint de descubrimiento del hold, nunca implementado.** `FD-296:89-92` lo especifica textualmente ("Mantén TAB para cambiar de pantalla", se apaga tras el primer uso) porque *el hold es invisible*. No existe ninguna línea que lo haga. | No existe | cero ocurrencias en `core_v2/ui/hud/*.gd` | **P1** |
| P2-7 | **Onboarding del HUD: las piezas existen sin cablear.** `ProtocolOverlay` ya tiene `BtnClose`+`BtnNext` (o sea, soporta secuencias), `PlayerHintManager.show_manual_hint()` y `SubtitlesOverlay.show_hint()` existen, y `FirstRunConsent` es el patrón de "primera vez". Ninguno está conectado al HUD. | Piezas sueltas | `core_v2/ui/overlays/ProtocolOverlay.gd:10-11,21` | P2 |
| P2-8 | **5 fuentes huérfanas** en `assets/fonts/` con cero referencias: `Silkscreen-Regular`, `Silkscreen-Bold`, `Jersey15-Regular`, `GeistPixel_Regular`, `Doto-VariableFont`. | Muerto | grep sin resultados | P2 |
